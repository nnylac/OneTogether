...
export function createIncidentLogEntries(
  ...
  const medicalResource = resources.find(
    (resource) => resource.type === "Medical Team",
  );
  const supportResource = resources.find(
    (resource) => resource.agency === "SPF",
  );
  ...
  return [
    ...
    {
      agencyId: medicalResource?.agency ?? "MOH",
      id: "log-004",
      category: "medical",
    ...
    {
      agencyId: supportResource?.agency ?? primaryResource?.agency ?? "SCDF",
      id: "log-003",
      category: "deploy",
      ...
      time: getTime(
        supportResource?.assignedAt ??
          primaryResource?.assignedAt ??
          incident.date,
      ),
    },
    {
      agencyId: primaryResource?.agency ?? "SCDF",
      id: "log-002",
      category: "status",
